...
############################### Agregado por mi ###################################################
def calculate_histogram(channel_l):
    # Calcula el histograma del canal L
    histogram = torch.histc(channel_l, bins=256, min=0, max=1)
    return histogram


def equalize_histogram(channel_l):
    # Calcula el histograma del canal L
    histogram = calculate_histogram(channel_l)
    # Calcula la CDF (Cumulative Distribution Function)
    cdf = histogram.cumsum(dim=0)
    cdf = (cdf - cdf.min()) / (cdf.max() - cdf.min())  # Normaliza la CDF

    # Convierte a NumPy para usar numpy.interp
    channel_l_np = channel_l.cpu().numpy().flatten()
    cdf_np = cdf.cpu().numpy()

    # Interpolación utilizando NumPy
    channel_l_equalized_np = np.interp(channel_l_np, np.linspace(0, 1, 256), cdf_np)

    # Convertir de nuevo a tensor de PyTorch
    channel_l_equalized = torch.from_numpy(channel_l_equalized_np).reshape(channel_l.shape)

    '''# Imprimir histograma y CDF
