...
from gui import gui_main_gt
import modeling


def get_args():
    parser = argparse.ArgumentParser('Colorization UI GT hints', add_help=False)
    parser.add_argument('--model_path', type=str, default='path/to/checkpoints', help='checkpoint path of model')
    parser.add_argument('--target_image', default='path/to/image', type=str, help='image used both as GT source and L target')
    parser.add_argument('--hints_file', default=None, type=str, help='optional hints file with GT hint coordinates')
    ...
    parser.add_argument('--rank', type=int, default=32, help='Rank for LoRA model')
...
if __name__ == '__main__':
    ...
    ex = gui_main_gt.IColoriTGTUI(
        color_model=model,
        img_file=args.target_image,
    ...
    ex.setWindowTitle('iColoriT GT Hints')
    ex.show()
    sys.exit(app.exec_())
